...
module.exports = {
  ...
  turnDevOff: function(package) {
    // Note that the module code cannot be modified directly since the
    // object has been frozen at this point.
    var devOffCode = '__DEV__ = false; \n' +
                     '__BUNDLE_START_TIME__ = Date.now();';

    var devOffModule = {code: devOffCode};
    var modifiedModules = package._modules.filter(function(moduleItem) {
      return !moduleItem.sourcePath.includes('prelude_dev');
    });

    modifiedModules.unshift(devOffModule);
    package._modules = modifiedModules;
  },
};
